    ...
    public class UpdateEventValidator : AbstractValidator<EventDto>
    {
        public UpdateEventValidator()
        {
            RuleFor(x => x.Id)
                  .NotNull()
                  .NotEmpty()
                  .WithMessage(x => string.Format("Id must have a value valid", nameof(x.Id)));

            RuleFor(x => x.Subject)
                  .NotNull()
                  .NotEmpty()
                  .WithMessage(x => string.Format("The subject must have a value valid", nameof(x.Subject)));

            RuleFor(x => x.Body)
                  .NotNull()
                  .NotEmpty()
                  .WithMessage(x => string.Format("The pageSize can't be less than zero", nameof(x.Body)));
